...
const SideNav = () => {
  ...
  return (
    <aside className="w-full fixed md:static flex md:w-80 justify-between items-center md:justify-start md:items-start  md:flex-col md:pb-10 bg-white shadow-md p-6">
      ...
      <div onClick={() => navigate("/dashboard")} className="flex items-center gap-2 font-bold text-2xl cursor-pointer text-lg md:text-2xl font-bold text-black md:mb-8 tracking-tighter">
      ...
      <ul className="w-[75%] hidden md:flex items-center md:items-start md:mb-60 justify-between md:gap-6 md:flex-col text-gray-700 font-medium">
        <li onClick={() => navigate("/dashboard")} className="cursor-pointer hover:text-blue-500">Dashboard</li>
      ...
      <div
        className={`w-full h-full md:hidden bg-black/20 shadow-2xl fixed bottom-0 z-50 ${nav ? "left-0" : "-left-full"
      ...
      >
        <ul className="w-72 h-full bg-white flex flex-col gap-6 pt-10 px-2  ">
          ...
          <li onClick={async () => await logout()} className="flex justify-end">
      ...
      <div className="md:hidden cursor-pointer z-50">
        ...
        >
          <div
            className={`w-full h-0.5 bg-white duration-500 ${nav ? "-rotate-45 translate-y-2" : ""
              }`}
          ></div>
          <div
            className={`w-full h-0.5 bg-white duration-500 ${nav ? "opacity-0" : "opacity-100"
              }`}
          ></div>
          <div
            className={`w-full h-0.5 bg-white duration-500 ${nav ? "rotate-45 -translate-y-2" : ""
      ...
      <Button onClick={async () => await logout()} className="bg-red-600 w-full hidden md:block rounded-3xl py-1 px-2">
        logout
      </Button>
    </aside>
  );
